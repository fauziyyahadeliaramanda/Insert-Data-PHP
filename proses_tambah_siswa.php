...
if($_POST){
    ...
    if(empty($nama_siswa)){
        ...
        echo"<script>alert('username tidak bileh kosong');location.href='tambah_siswa.php;</script>";
    }elseif(empty($password)){
        echo"<script>alert('password tidak bileh kosong');location.href='tambah_siswa.php;</script>";
    }else{
        include "Koneksi.php";
        $insert=mysqli_query($conn,"insert into siswa(nama_siswa,tanggal_lahir,gender,alamat,username,password,id_kelas,id_jurusan)
        value
        ('".$nama_siswa." ','".$tanggal_lahir." ','".$gender." ','".$alamat." ','".$username." ','".md5($password)." ','".$id_kelas." ','".$id_jurusan." ')")or
